...
Job directory (durable state, the source of truth):
  <root>/<skill>/<run-id>/jobs/<job-id>/
    meta.json   canonical receipt ref/hash, requested/actual peer identity,
...
Environment overrides (defaults in parentheses):
  SPEC_FIRST_PEER_JOBS_ROOT         base dir (/tmp/spec-first-<effective-uid>)
  SPEC_FIRST_WORK_RUNS_ROOT         parent spec-first Work dir containing all <run-id>/ dirs
  SPEC_FIRST_PEER_IDLE_SECS         idle window, no out.log growth (240)
  SPEC_FIRST_PEER_HARD_SECS         hard cap on worker wall clock (630)
  SPEC_FIRST_PEER_LOG_MAX_BYTES     out.log byte cap (10485760)
  SPEC_FIRST_PEER_RESULT_MAX_BYTES  result byte cap, supervise + read (5242880)
  SPEC_FIRST_PEER_POLL_SECS         supervisor poll interval (2)
  SPEC_FIRST_PEER_GRACE_SECS        TERM-to-KILL grace during reap (5)
...
exec'd directly (argv list, never a shell) but is never persisted; job
...
POSIX path is behaviorally unchanged:
  ...
  jobs root defaults under %LOCALAPPDATA%\\spec-first-peer-jobs (then the
...
if IS_WINDOWS:
    ...
    DEFAULT_ROOT = os.path.join(_WIN_ROOT_BASE, "spec-first-peer-jobs")
elif _EFFECTIVE_UID is not None:
    DEFAULT_ROOT = os.path.join("/tmp", f"spec-first-{_EFFECTIVE_UID}")
else:
    DEFAULT_ROOT = None
...
SAFE_WORKER_ENV = (
    'PATH', 'HOME', 'USERPROFILE', 'LOCALAPPDATA', 'TMPDIR', 'TEMP', 'TMP',
    'SystemRoot', 'COMSPEC', 'LANG', 'LC_ALL', 'TERM',
)
...
environment overrides: SPEC_FIRST_PEER_JOBS_ROOT, SPEC_FIRST_WORK_RUNS_ROOT, SPEC_FIRST_PEER_IDLE_SECS,
SPEC_FIRST_PEER_HARD_SECS, SPEC_FIRST_PEER_LOG_MAX_BYTES, SPEC_FIRST_PEER_RESULT_MAX_BYTES,
SPEC_FIRST_PEER_POLL_SECS, SPEC_FIRST_PEER_GRACE_SECS (defaults in the module docstring).
"""
...
def jobs_root_base() -> str:
    configured = os.environ.get("SPEC_FIRST_PEER_JOBS_ROOT")
    if configured:
...
def skill_runs_root(skill: str) -> str:
    if skill == "spec-work" and os.environ.get("SPEC_FIRST_WORK_RUNS_ROOT"):
        return os.path.abspath(os.environ["SPEC_FIRST_WORK_RUNS_ROOT"])
...
def cfg(skill=None) -> dict:
    return {
        "idle": _env_num("SPEC_FIRST_PEER_IDLE_SECS", 240.0, float, allow_zero=skill == "spec-work"),
        "hard": _env_num("SPEC_FIRST_PEER_HARD_SECS", 630.0, float),
        "log_max": int(_env_num("SPEC_FIRST_PEER_LOG_MAX_BYTES", 10 * 1024 * 1024, int)),
        "result_max": int(_env_num("SPEC_FIRST_PEER_RESULT_MAX_BYTES", 5 * 1024 * 1024, int)),
        "poll": _env_num("SPEC_FIRST_PEER_POLL_SECS", 2.0, float),
        "grace": _env_num("SPEC_FIRST_PEER_GRACE_SECS", 5.0, float),
    }
...
def _load_owned_json(path: str, cap: int, label: str):
    try:
        raw = read_owned(os.path.abspath(path), cap)
        return raw, json.loads(raw.decode('utf-8'))
    except (OSError, Unreadable, UnicodeDecodeError, ValueError) as exc:
        raise RunnerError(f'{label} is not an owner-checked JSON file: {exc}')
...
def redact_private_log(job_dir: str, credential_env) -> None:
    """Prevent secret-like values from surviving in captured stdout/stderr."""
    path = os.path.join(job_dir, 'out.log')
    try:
        raw = read_owned(path, cfg()['log_max'])
        ...
        # A terminal record remains usable even when the private diagnostic log
        # cannot be sanitized; consumers must never treat that log as durable.
        try:
            write_atomic(path, b'[REDACTION_UNAVAILABLE]\n')
        except OSError:
            pass


def validate_start_authority(args, worker_argv):
    """Validate safety facts without redefining their canonical semantics."""
    _require_sha256(args.authorization_receipt_sha256, '--authorization-receipt-sha256')
    receipt_raw, receipt = _load_owned_json(
        args.authorization_receipt, META_READ_CAP, 'authorization receipt')
    if sha256_bytes(receipt_raw) != args.authorization_receipt_sha256:
        raise RunnerError('authorization receipt SHA-256 mismatch')
    if receipt.get('schema_version') != 'worker-dispatch-host-journey/v1':
        raise RunnerError('authorization receipt is not a canonical worker-dispatch journey')
    if receipt.get('worker_dispatch_capability') != 'available':
        raise RunnerError('canonical receipt does not establish dispatch availability')
    if not receipt.get('dispatch_authorization_receipt'):
        raise RunnerError('canonical receipt has no dispatch authorization source ref')
    if receipt.get('authorization_basis') not in ('explicit-user', 'visible-upstream-handoff'):
        raise RunnerError('canonical receipt has no accepted authorization basis')
    expires_at = receipt.get('freshness_expires_at')
    try:
        expires_epoch = datetime.datetime.fromisoformat(
            expires_at.replace('Z', '+00:00')
        ).timestamp()
    except (TypeError, ValueError, OverflowError):
        raise RunnerError('canonical receipt freshness timestamp is invalid')
    if expires_epoch <= time.time():
        raise RunnerError('canonical receipt is stale')
    if receipt.get('redaction_status') != 'passed':
        raise RunnerError('canonical receipt redaction did not pass')
    if receipt.get('provider_trust_domain') != args.provider_trust_domain:
        raise RunnerError('provider trust domain does not match the canonical receipt')

    semantic_ref = receipt.get('semantic_request_ref')
    semantic_sha = receipt.get('semantic_request_sha256')
    _require_sha256(semantic_sha, 'semantic_request_sha256')
    request_raw, request_doc = _load_owned_json(
        semantic_ref, META_READ_CAP, 'canonical semantic request')
    if sha256_bytes(request_raw) != semantic_sha:
        raise RunnerError('canonical semantic request SHA-256 mismatch')
    request = request_doc.get('worker_dispatch_request')
    if not isinstance(request, dict) or request.get('worker_dispatch_authorization') != 'authorized':
        raise RunnerError('canonical semantic request does not authorize dispatch')

    authorization_fields = (
        'restricted_read_authorization', 'data_egress_authorization',
        'credential_use_authorization', 'external_communication_authorization',
    )
    for field in authorization_fields:
        if request.get(field) != receipt.get(field):
            raise RunnerError(f'{field} does not match the canonical receipt')
    if args.provider_trust_domain in ('external', 'unknown'):
        for field in authorization_fields:
...
def ensure_owned_dirs(base: str, path: str) -> None:
    ...
    if IS_WINDOWS:
        ...
        # SPEC_FIRST_PEER_JOBS_ROOT keeps its ACLs and rests on the owner check.
...
def _popen_argv(argv):
    ...
    worker must be launched through bash/sh. Already-prefixed workers (review
    skills use `bash script.sh`) are left alone.
    """
...
def supervise(job_dir: str, argv, result_path, conf: dict, ack_fd: int) -> None:
    ...
    try:
        ...
        try:
            ...
            allowed_env = set(SAFE_WORKER_ENV) | set(conf.get('credential_env', []))
            worker_env = {
            ...
            worker_env.update({
                "SPEC_FIRST_PEER_JOB_ID": os.path.basename(job_dir),
                "SPEC_FIRST_PEER_PYTHON": sys.executable,
            })
            popen_kwargs = dict(
                stdin=devnull,
                stdout=log_fd,
            ...
            # Wrap bare *.sh on Windows at spawn time only.
    ...
    try:
        ...
        pass
    redact_private_log(job_dir, conf.get('credential_env', []))
...
def _require_detach_support() -> None:
    ...
    if not hasattr(os, "fork") or not hasattr(os, "setsid"):
        raise RunnerError(
            ...
            "EveryInc/spec-first-plugin#1243)."
        )
...
def cmd_start(args, worker_argv) -> int:
    ...
    authority = validate_start_authority(args, worker_argv)
    ...
    conf = cfg(args.skill)
    conf['credential_env'] = authority['credential_env']
    meta = {
        "job_id": job_id,
        "run_id": args.run_id,
        ...
        "payload_byte_count": authority['payload_bytes'],
        "payload_sha256": authority['payload_sha256'],
        "payload_redaction_status": args.payload_redaction_status,
        "result_path": result_path,
        "log_ref": os.path.join(job_dir, 'out.log'),
        "limitations": [],
    }
    try:
        create_exclusive(
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_start.add_argument('--source-identity', required=True)
    p_start.add_argument('--provider-trust-domain', required=True, choices=('host-native', 'external', 'unknown'))
    p_start.add_argument('--host-provider', required=True)
    p_start.add_argument('--requested-provider', required=True)
    p_start.add_argument('--actual-provider', required=True)
    p_start.add_argument('--requested-model', required=True)
    p_start.add_argument('--actual-model', required=True)
    p_start.add_argument('--credential-env', action='append', default=[])
    ...
    p_start.add_argument(
        ...
        help="retain old sibling run roots (spec-work durable recovery)",
    )
