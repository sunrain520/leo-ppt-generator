...
# Prepare and launch one governed cross-model peer job for spec-doc-review.
...
worker_main() {
  ...
  jq --arg reviewer "whole-doc-$peer" '
...
PERSONA="$SKILL_ROOT/references/personas/whole-doc-reviewer.md"
...
DOCUMENT="$(cd "$(dirname "$DOCUMENT")" 2>/dev/null && pwd -P)/$(basename "$DOCUMENT")"
[ -f "$DOCUMENT" ] || fail "document is unavailable"
...
jq -e --arg document "$DOCUMENT" 'index($document) != null' "$INPUT_REFS_FILE" >/dev/null ||
  fail "document is not allowlisted by the canonical semantic request"

{
  cat "$PERSONA"
  printf '\n\n---\n\n'
  printf '%s\n' 'This is an authorized report-only whole-document review.'
  ...
  cat "$SCHEMA"
  printf '\n\nSet the top-level "reviewer" field to "whole-doc-%s".\n' "$PEER"
...
OUT="$RUN_DIR/whole-doc-$PEER.json"
