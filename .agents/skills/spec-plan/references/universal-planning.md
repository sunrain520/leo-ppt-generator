...
Carry out the approach. When the answer depends on facts the model can't reliably supply from memory — current data, recent events, specifics that drift — gather them using the **Research decomposition pattern** under Step 1 below. Decompose the work into focused questions and request bounded generic research workers only when the main Skill's boundary has `worker_dispatch_authorization: authorized`, current-session `worker_dispatch_capability: available`, the required data-access authorization, and sufficient bounded-parallelism facts. Otherwise research serially with the boundary's exact fallback reason and coverage limitation, then collate the results. Skip research for anything the model already knows well.
...
| **Recommended** | Plan references specific locations, venues, dates, prices, schedules, seasonal availability, or current events — anything where stale information would break the plan (closed restaurants, changed prices, cancelled events, wrong seasonal dates). | Research before planning. Decompose into 2-5 focused research questions and request bounded generic research workers only when authorization, current-session capability, data authorization, and parallelism facts permit it. Request the extraction tier only when model override is supported; otherwise inherit. Fall back to serial research and collate findings before structuring the plan. |
