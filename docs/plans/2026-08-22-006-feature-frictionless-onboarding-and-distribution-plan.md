...
- **决策焦点：** 用户界面与机器接口分层、Python 3.12 私有受管安装、host capability 真值、Provider 选择与确认、安全凭据存储、Plugin/standalone 单源发布、升级兼容。
...
- R4. 用户级默认发现目录与当前 Codex 官方支持保持一致；兼容旧目录时必须检测重复副本并给出唯一迁移动作，不得同时激活多个同名 Skill。
- R5. Plugin 与 standalone Skill 必须从同一 canonical bundle 构建，内容 hash 一致；任何渠道不得维护功能分叉。

#### Setup and progressive disclosure

- R6. 系统提供一个稳定 setup facade，组合已有 `ensure`、route `doctor`、backend registry 和 credential status，不复制其验证规则或持久化第二套 readiness 状态。
...
- R9. OpenAI 与 AtlasCloud 为图片 Provider 选择关系，不要求同时配置；PaddleOCR 只在可编辑路径实际需要在线 OCR 时延迟询问，未配置时明确使用本地降级能力。
- R10. 普通用户主流程不得要求手写 backend JSON、定位 CLI、理解 runtime identity 或读取完整 doctor JSON；这些内容只出现在高级排障和机器接口中。

#### Credentials and security

- R11. 已存在的允许环境变量继续受支持；新增凭据设置入口必须使用隐藏输入，并将 secret 写入 macOS Keychain 或 Windows DPAPI 保护的用户级存储。
- R12. backend contract、run、日志、receipt、错误文本和遥测只保存 `credential_ref` 与状态，不保存值、长度、前后缀或 secret hash。
...
- R17. 每个阻断状态必须拥有一个 primary action；替代路径只能在高级详情中展示，避免一次抛出多条互相竞争的修复建议。
- R18. 安装结束必须给出一条可直接发送给 Agent 的首次使用示例；首次生成必须在完整制作前经过已有样张确认合同。

#### Compatibility, rollout, and evidence

- R19. 现有 `ensure`、`doctor`、backend create/validate、run lifecycle 和四 route CLI 保持兼容；setup facade 不能成为新的领域真值 owner。
- R20. 现有手工安装、固定版本、升级、卸载和 JSON 自动化接口继续工作；废弃路径至少经历一个有警告的兼容周期。
...
- F2. **外部 Provider 首次生成**
  ...
  - **Steps:** setup 只展示 OpenAI/AtlasCloud 二选一；A1 在本地隐藏输入中添加凭据；状态检查返回 available；Agent 创建非敏感 contract 并继续样张确认。
  - **Outcome:** 聊天、日志、run 和 backend JSON 中不存在 secret，任务成功或给出唯一恢复动作。
  - **Covers:** R8-R18、R21。
...
- F4. **凭据失效恢复**
  ...
  - **Steps:** setup/provider smoke 返回稳定 reason code；Agent 只提示重新设置当前 Provider；旧 run 和已完成产物保持不变。
  - **Outcome:** 用户修复后从安全 checkpoint 恢复，不重复创建 run 或切换 Provider。
...
- KTD8. **安全交互优先。** `auth add` 仅在交互式 TTY 中读取隐藏输入；非 TTY 拒绝从参数、stdin pipe 或聊天接收 secret。Agent 无法建立安全 TTY 时，只能指导用户在本地终端运行一条命令。
- KTD9. **人类输出与机器输出同源。** setup 先构造 `setup-report/v1`，再分别渲染 concise human 和 JSON；reason code、primary action 和状态不得维护两份逻辑。
- KTD10. **单一 primary action。** 每个阻断 reason code 映射一个稳定 `primary_action`，可选替代方案只在 `details.alternatives` 中出现；安装器和 Agent 均消费同一映射。
...
stateDiagram-v2
  ...
  Evaluating --> Ready: 有唯一可用 backend
  ...
  ChoiceRequired --> Ready: 用户确认
  CredentialRequired --> Evaluating: 安全添加凭据
  HostCheckRequired --> Evaluating: 宿主声明能力
  Ready --> [*]
```

`Ready` 只表示可开始流程，不表示真实 Provider 已 smoke，也不表示最终 PPT 已验收。
...
- `status`: `ready|choice_required|action_required|blocked`
...
- `primary_action`: 阻断时恰好一个稳定动作；ready 时为空
...
- setup 默认不做付费 Provider 调用；Provider smoke 仍在真实 run 执行前单独完成并生成 receipt。
...
- Phase A：U1-U4 建立合同、bootstrap 和凭据基础，但不切换默认文档入口。
...
- **Behavior:** 支持 route、host capability facts、human/JSON 模式；输出状态、候选 Provider、选择结果或唯一 primary action；保留原始 evidence refs。
- **Test scenarios:** host available/unavailable/unknown；0/1/2 个外部 Provider 可用；非法 route；config blocked；credential resolver unavailable；每个 blocked reason 恰有一个 primary action；human 与 JSON golden semantic parity；schema version 不兼容 fail closed。
...
- **Behavior:** 复用兼容解释器；否则只从 manifest allowlist 下载并校验固定 bootstrap 工件，在 Leo home 安装私有 Python；全程无需管理员权限；阶段进度可见；网络/校验/磁盘错误保持旧版；安装前扫描官方默认目录和兼容旧目录，发现同名活动副本时阻断并返回唯一迁移动作。
- **Test scenarios:** system Python 命中；已有 uv 命中；完全缺失自动安装；不兼容架构；非 HTTPS/非 allowlist origin、超大小、下载 404、超时、截断、SHA mismatch；代理失败；只读 home；磁盘满；并发 bootstrap；中断重试；升级后复用；默认目录、仅旧目录、双目录和可发现 backup 目录；PATH、系统 Python 和其他 venv 前后不变。
...
- **Behavior:** 只有明确 available 才选择 host backend；外部 Provider 按 capability、credential status 和用户既有确认排序；OCR 不参与图片 Provider 选择；切换 Provider 保持样张重新确认合同。
- **Test scenarios:** 四 route capability matrix；host unknown 不选择；host unavailable + OpenAI available；两个外部 Provider 同时可用要求选择；仅 Atlas 在 mask route 不足时阻断；环境中出现新 secret 不改变已冻结 run；OCR 缺失不阻断 generate；在线 OCR 仅在相关 editable 场景出现。
...
- **Behavior:** `auth add/status/remove` 采用 provider allowlist；add 只接受 TTY 隐藏输入；macOS Keychain item 绑定固定 service 和 provider account；Windows DPAPI 使用 current-user scope，密文容器目录/文件限制当前用户 ACL；环境变量兼容；状态输出不含 secret 特征。
- **Test scenarios:** add/status/overwrite-confirm/remove；非 TTY、CLI 参数、pipe 输入拒绝；Keychain locked/denied/missing 和错误 service/account；DPAPI encrypt/decrypt、用户身份不符、ACL 过宽、blob 损坏；环境变量优先级；删除后执行稳定失败；异常、receipt、日志、run、core dump fixture 的 secret scan；同用户进程攻击明确记录为不在保护上限内。
...
- **Behavior:** 首次调用自动选择平台 launcher；普通用户不看 `SKILL_DIR`、`print-cli` 或 backend JSON；host unknown 时 Agent 先核实能力；需要 secret 时禁止聊天粘贴并给出单一终端动作；完成后回到原任务。
- **Test scenarios:** 零密钥生成；缺 Provider；多个 Provider；用户拒绝外部服务；OCR 延迟；Windows launcher；旧 runtime 复用；setup blocked；用户要求跳过样张；Provider 切换；Agent 不得安装额外历史 CLI；所有 case 检查引用最小化与 next action 唯一性。
...
- **Behavior:** README 首屏依次展示 Plugin/Agent 安装、首次使用示例、零密钥/一个图片密钥/可选 OCR 三档；高级命令移出主路径；macOS/Windows 示例对等；错误入口按 primary action 索引。
- **Test scenarios:** 所有链接可达；命令来自真实 CLI help；平台示例 parity；禁止历史项目信息；密钥申请入口和安全警告存在；README 不要求普通用户手写 backend JSON；兼容、限制和现场证据声明一致。
...
- **Test scenarios:** macOS/Windows × system/private Python × standalone/Plugin × zero-key/OpenAI/Atlas × 四 route；重复安装、双目录、网络抖动、限流、凭据撤销、OS store 锁定、磁盘满、进程中断、长路径、空格/中文路径、代理、升级失败、旧版回滚、日志泄露、宿主 capability unknown、Provider smoke 失败、人工拒绝样张。
...
| G2 Deterministic behavior | setup、bootstrap、selection、credential、reason code | unit/integration/boundary tests | 机制不可信 |
...
- `tests/unit/test_setup.py`：setup 状态、Provider 排序、primary action 和 human/JSON parity。
...
- `tests/journeys/`、`tests/release/test_installed_routes.py`：不绕行的安装后首次成功与四 route。
...
- 保留现有 direct `runtime_manager.py ensure/doctor/print-cli` 与 `leo-ppt backend/run/...` 接口；setup 上线不要求迁移已有 run。
- launcher 或 setup 失败时，支持人员仍可使用已安装 current runtime；不得自动删除旧 runtime、OS store credential 或 run。
- 文档主路径切换前先在发布候选中验证；若 field failure 超过阈值，回退 README/SKILL 默认入口，但保留已发布的新接口供诊断。
...
- D5. OpenAI/AtlasCloud 选择和 PaddleOCR 延迟配置行为满足 route capability，Provider 切换继续触发样张重新确认。
...
- D9. 所有 blocked setup/install reason 只有一个 primary action，并在执行该动作后有明确复验条件。
- D10. 现有 deterministic、integration、release、skill eval、upstream 和四 route installed tests 全绿，无兼容回归。
