...
- **Test scenarios:**
  ...
  6. Ledger 中每个 adapted/retired 条目若其 disposition 与首次同步不同（通过 git history 或 ledger 自带 `disposition_history` 字段判断），必须有 inline reason 或独立 changelog 说明变更理由；缺失时 `verify_upstreams.py --check` 失败。首次同步的 disposition 无需理由。
