...
graphify clone <url1>   # → ~/graphify-out/repos/<owner1>/<repo1>
graphify clone <url2>   # → ~/graphify-out/repos/<owner2>/<repo2>
# Run /graphify on each local path to produce their graph.json files
# Then merge:
graphify merge-graphs \
  ~/graphify-out/repos/<owner1>/<repo1>/graphify-out/graph.json \
  ~/graphify-out/repos/<owner2>/<repo2>/graphify-out/graph.json \
  --out graphify-out/cross-repo-graph.json
```

Graphify clones into `~/graphify-out/repos/<owner>/<repo>` and reuses existing clones on repeat runs. Each node in the merged graph carries a `repo` attribute so you can filter by origin.
