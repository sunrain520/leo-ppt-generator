...
- **File-size regression** — a touched file crossing **1000 lines** because of this diff, or growing materially without decomposition. Flag at **P1** when the diff pushes a file from under 1k to over 1k; at **P2** when already over 1k and the diff adds substantial surface without splitting. This is a persona-defined mechanical threshold, not a subjective “file getting long” concern: keep its persona-assigned severity and confidence when before/after line-count evidence and the added diff prove the crossing, even when project standards do not repeat the threshold or the shared advisory rule would otherwise treat structural cost as non-runtime impact.
