...
**Grounding freshness.** Phase 1 elsewhere-mode grounding has already run before this reference takes over: user-context synthesis always supplies the local basis, web research appears only when the main Skill recorded `external_research_authorization: authorized`, and learnings are skipped for non-software (see SKILL.md Phase 1). Its outputs feed the generation below. If intake answers materially refine the topic or constraints, rerun the affected Phase 1 roles on the refined topic before generating ideas. Dispatch them only when the main Skill's authorization boundary permits it; missing external research authority still removes the web role even when worker dispatch is authorized.
...
Generate the full candidate list before critiquing any idea. Use the same six frames as software ideation, described in domain-agnostic language. Each frame is a **starting bias, not a constraint** — follow promising threads across frames. In Full depth, dispatch frames as parallel sub-agents only when the main Skill's boundary permits it; otherwise execute all six frame lenses serially inline. For authorized dispatch, follow SKILL.md Model Tiers.
...
Before the final cut, run one basis-verification role (generation tier — see SKILL.md Model Tiers) against only the grounding summary and candidate list, prompted to refute unsupported bases, false or irrelevant prior art, and unsound reasoning. Dispatch it as a fresh-context verifier only when the main Skill's authorization and capability boundary permits; otherwise run the same rubric inline or serially, preserve the matching fallback reason, and do not claim independent verification. In this mode verification uses user-supplied context and web research — no repo reads. Weigh the verdicts in the cut, overruling with stated reasons.
...
- **Persist the deliverable when a durable owner exists** (Phase 4.1). The full per-idea detail and rejection summary go in `docs/ideation/` when that repo-owned directory already exists. Otherwise return the complete deliverable inline and offer a user-selected durable destination; never make run-local scratch or an arbitrary CWD the only copy. `OUTPUT_FORMAT` (SKILL.md Phase 0.0; default `html`) sets the rendered form.
...
Then offer the Phase 5 next-steps menu via the platform's blocking question tool (`AskUserQuestion` in Claude Code — call `ToolSearch` with `select:AskUserQuestion` first if its schema isn't loaded; `request_user_input` in Codex). Fall back to a numbered list only when no blocking tool exists or the call errors. Never silently skip. Offer the applicable options and renumber them contiguously:

1. **Open in browser** *(html only)* — open the HTML deliverable (per §5.1).
2. **Brainstorm one idea with `spec-brainstorm`** — go deeper on one chosen idea (asks which). In universal mode this is not an automatic implementation chain: `spec-ideate` hands the selected idea only to `spec-brainstorm` and must not jump directly to planning. `spec-brainstorm` develops the idea further (a name into a brand brief, a plot into an outline, a decision into a weighed framework); if the user then explicitly chooses **Create a plan** in the `spec-brainstorm` wrap-up, that workflow may hand the settled goal and constraints to universal/knowledge-work `spec-plan`. That universal plan does not offer `spec-work`; only a later artifact that genuinely becomes a complete software implementation plan may enter the separate software execution chain. Seed brainstorm with the idea's substance + a provenance pointer (per §5.2) — not the whole file.
