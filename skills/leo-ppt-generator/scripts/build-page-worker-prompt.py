...
def run_dir_from_target(target: str) -> Path:
    ...
    if path.is_dir():
        embedded = path / "editable" / "upstream"
        canonical = path / "editable"
        if (path / "run.json").is_file() and (canonical / "page_jobs.json").is_file():
            return canonical
        # 兼容直接传入 vendor 归一化目录，但公开 run 只应使用
        # editable/page_jobs.json 这一份 canonical state。
        if (path / "run.json").is_file() and (embedded / "page_jobs.json").is_file():
            return embedded
        return path
...
def page_dir_for(run_dir: Path, page: dict) -> Path:
    ...
    resolved_page = page_dir.resolve() if page_dir.is_absolute() else (resolved_run / page_dir).resolve()
    try:
        resolved_page.relative_to(resolved_run)
    except ValueError as exc:
        raise SystemExit(f"Page directory must live inside run dir: {resolved_page}") from exc
...
def resolve_cli(value: str) -> Path:
    cli = Path(value).expanduser()
    if not cli.is_absolute():
        raise SystemExit("--cli must be the absolute path returned by runtime_manager.py print-cli")
    resolved = cli.resolve()
    if not resolved.is_file() or not os.access(resolved, os.X_OK):
        raise SystemExit(f"--cli must identify an executable file: {resolved}")
    return resolved


def build_prompt(run_dir: Path, page: dict, page_dir: Path, cli: Path) -> str:
...
def main() -> int:
    ...
    parser.add_argument(
        "--cli",
        required=True,
        help="Absolute leo-ppt executable path returned by runtime_manager.py print-cli.",
    )
    ...
    out.write_text(build_prompt(run_dir, page, page_dir, cli).strip() + "\n", encoding="utf-8")
    ...
    payload = {
        ...
        "dispatch_command_template": (
            f"{shlex.quote(str(cli))} upstream editable-ppt -- run dispatch "
            f"{shlex.quote(str(run_dir))} --page {shlex.quote(str(page_id))} "
