...
Owner: created by `leo-ppt upstream editable-ppt -- prepare`; `leo-ppt upstream editable-ppt -- run backend` may update the image backend; `leo-ppt upstream editable-ppt -- run finalize` reads it and writes completion time.
...
```json
{
  ...
  "image_backend": {
    ...
    "save_path_policy": "use only an explicit valid local result/output_hint path, then leo-ppt upstream editable-ppt -- image import; never scan for a newest file",
    "fallback_command": "leo-ppt upstream editable-ppt -- image generate/edit",
...
- `save_path_policy`: permits only an explicit valid local result path, including `output_hint`, followed by `leo-ppt upstream editable-ppt -- image import`; newest-file directory scanning is forbidden.
...
Owner: created by `leo-ppt upstream editable-ppt -- prepare`, updated by `leo-ppt upstream editable-ppt -- run` commands.
...
`dispatch` is written by `leo-ppt upstream editable-ppt -- run dispatch`. It includes `execution_mode`: `"worker"` for normal page-worker dispatch and `"local"` for the parent agent's single-page local claim; older dispatch records without this field are treated as `"worker"`. A page with status `dispatched` is an active execution lease until explicit completion, failure, cancellation, or lost-worker verification; elapsed time alone does not make it lost. `result` is written by `leo-ppt upstream editable-ppt -- run record`. `accepted` is written by `leo-ppt upstream editable-ppt -- run finalize`.

## `page_request.json`

Owner: `leo-ppt upstream editable-ppt -- prepare`.
...
`slide` and `content_box` are computed automatically by `leo-ppt upstream editable-ppt -- prepare`. Inputs close to 16:9 use the standard widescreen canvas; other inputs use a custom canvas converted from the source image pixel dimensions. The agent must copy these two fields into the page `manifest.json` and must not compress, stretch, or recalculate the canvas.

## `page_result.json`

Owner: created by the page reconstructor, validated by `leo-ppt upstream editable-ppt -- run record`.
...
- contact sheet path
- validation path
- page-local output hashes, which may be supplemented by `leo-ppt upstream editable-ppt -- run record`
...
The `manifest` artifact is the authoritative page source for final assembly. `leo-ppt upstream editable-ppt -- run finalize` rebuilds the final deck from recorded page manifests in page order. The `page_pptx` artifact remains a page-level deliverability artifact and is validated by `leo-ppt upstream editable-ppt -- run record`, but it is not the final assembly input.

## `pages/page_NNN/validation.json`

Owner: created by the page reconstructor, read by `leo-ppt upstream editable-ppt -- run record`.
...
`passed` must be a boolean. `leo-ppt upstream editable-ppt -- run record` only reads top-level `passed` to decide whether the page can enter final assembly. `status: "pass"`, `runtime_validation.passed`, or other nested fields may remain as supplemental information, but they cannot replace top-level `passed`.
...
Missing coordinates are page-contract violations. The runtime must reject them during `leo-ppt upstream editable-ppt -- run record` and deck validation because otherwise missing values fall back to default positions such as the top-left corner.
  ...
  "asset_provenance": [
    {
      ...
      "provenance_note": "Rendered from LaTeX by leo-ppt upstream editable-ppt -- formula render-latex; visual fidelity is prioritized over formula editability."
    }
  ],
  "formula_inventory": [
    {
...
当输入源已由 prepare/用户确认形成公式真值时，页面还必须携带
`expected_formula_inventory`。它是 source-side 的冻结清单，每项至少包含稳定 `id`
和 `status: "confirmed"`；validator 会逐项要求 worker 的 `formula_inventory` 出现同一
`id`。低置信候选必须先确认，不能通过省略公式来绕过验证。

Formula images must be generated by `leo-ppt upstream editable-ppt -- formula render-latex`. Do not use source-image formula snippets, and do not assemble complex formulas from hand-written native text boxes.

## `pages/page_NNN/imagegen-jobs.json`

Owner: created by `leo-ppt upstream editable-ppt -- prepare`, updated by `leo-ppt upstream editable-ppt -- image import` and `leo-ppt upstream editable-ppt -- image process-sheet` (`generate`/`edit` do not write it — importing the selected output is what records the job).
...
Owner: created by `leo-ppt upstream editable-ppt -- prepare`, read by `leo-ppt upstream editable-ppt -- run finalize`.
