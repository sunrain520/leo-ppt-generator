...
- Common failure mode and page inventory
- Background recognition and repair
- Foreground asset separation
...
Do not guess font sizes or positions by eye — `leo-ppt upstream editable-ppt -- prepare` already measured them. Every page dir contains `text_hints.json` (each detected line's source-pixel `box_px`, glyph height, and derived font sizes; the `backend` field records which detector produced them) and `text_hints.png`, the source image with every detected line framed and labeled. If missing, regenerate with `leo-ppt upstream editable-ppt -- page hints <page_dir>`. Use the hints like this:
...
Transcribe each formula from the source into LaTeX, then render it with `leo-ppt upstream editable-ppt -- formula render-latex` into an image asset written inside the page directory (prefer SVG; use PNG when SVG preview/PowerPoint compatibility is unstable):

```bash
leo-ppt upstream editable-ppt -- formula render-latex <page_dir> \
