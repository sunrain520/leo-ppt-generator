...
- {{SKILL_ROOT}}/references/cli-helper.md — current editable command syntax and examples.
...
6. Record font substitutions, remeasure CJK/Latin text after substitution, and fail on overflow, occlusion, low contrast, canvas escape, or chart value/unit/label/order drift. A full-slide raster plus a few native text boxes is not editable reconstruction.

Deterministic CLI: `{{LEO_PPT}}`. This is the absolute executable path supplied by the parent from `runtime_manager.py print-cli`; use it for every deterministic command. Any bare `leo-ppt` text in fixed schema fields is a command-family label, not a PATH executable.

Image backend: execute `page_request.json.image_backend` using the authoritative field contract in `manifest-schema.md`. For `backend_id: builtin-imagegen`, the high-risk reminder is: use the host's built-in image generation tool first; generation needs only `prompt`, while editing requires viewing the source first and then passing the prompt plus absolute local referenced-image paths. Missing `mask`, `model`, `size`, `quality`, or `out` never triggers fallback. Import only the exact valid local result path (`output_hint` when supplied), never a scanned "newest" file; enter `"{{LEO_PPT}}" upstream editable-ppt -- image generate/edit` only for a matching `fallback_policy.on` event. If that fallback cannot produce the required image, stop the page with `validation.json.passed=false`. In a network-restricted runtime, request any required approval and state that only task-local prompts and required page images/masks/references are uploaded for this user-requested conversion.
...
3. Decide and separate foreground assets (section 2). Run step-1/2 image jobs serially through the backend order above; do not use a batch interface. Put icons/foreground objects onto one sparse asset sheet when they fit, with generous gaps between objects for clean splitting; create multiple sheets only when one sheet cannot fit them. After each selected local output, record and process it with `"{{LEO_PPT}}" upstream editable-ppt -- image import` and `"{{LEO_PPT}}" upstream editable-ppt -- image process-sheet`.
4. Rebuild native text, shapes, and tables (section 3). Fill `text_boxes` from the measured text hints per section 3.1; render formulas with `"{{LEO_PPT}}" upstream editable-ppt -- formula render-latex` per section 3.2.
...
6. Build the artifacts with the deterministic runtime: `"{{LEO_PPT}}" upstream editable-ppt -- page build {{PAGE_DIR}}` (writes page.pptx and preview.png from manifest.json), then `"{{LEO_PPT}}" upstream editable-ppt -- page contact-sheet {{PAGE_DIR}}`, then `"{{LEO_PPT}}" upstream editable-ppt -- page validate {{PAGE_DIR}}` — it runs the same manifest-contract checks `"{{LEO_PPT}}" upstream editable-ppt -- run record` will run, so fix every reported issue here, inside the page.
